class VonageAPI {
    ...
    try {
      if (this.isSlotFreeToMakeCalls && this.queue.isNotEmpty()) {
        ...
        if (callObj.callType === "conference") {
          ...
          setTimeout(async () => {
            try {
              console.log("Making Teacher Conference Call")
  ...
  makeVonageCall(phoneNumber, eventUrl, call_back_func, ncco) {
    console.log("Making Vonage Call")
  ...
  async makeTeacherCall(confId, numbers) {
    console.log("Making Teacher Call Inside Function");
    const {
    ...
    if (!conference.isEnded) {
      ...
      console.log("About to make Teacher Call");
      this.makeVonageCall(teacherNumber, eventUrl, call_back_func, ncco)
    } else {
      console.log("Conference is Ended in Function");
  ...
  async makeCallsToStudents(confId, numbers, startIndex, n) {
    ...
    if (!conference.isEnded) {
      console.log("Making Calls To Students");
      var ct = 0;
        ...
        if (user) {
          ...
          if (currentDate === user.date) {
            if (user.timeInMilliSeconds >= global.maxTimeLimitForUserInMilliseconds) {
              ...
              console.log("Time limit not exceeded...")
              console.log("milliSeconds", global.monoCallInfo[userPhoneNumber]['timeSpentInMilliSeconds'])
